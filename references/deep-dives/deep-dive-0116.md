# Distributed tracing на базе OpenTelemetry
...
1. **Первый принцип**: Масштабирование систем — это всегда серия компромиссов. Горизонтальное масштабир
2. **Второй принцип**: Надёжность системы определяется не средним временем безотказной работы, а средни
3. **Третий принцип**: Архитектурная эволюция — естественный процесс. Не существует идеальной архитекту
...
|---------|----------|-----------|-------------|
| Throughput | 2k RPS | 71k RPS | В 2x лучше |
| Latency p50 | 15ms | 24ms | Улучшение |
| Latency p99 | 309ms | 64ms | Значительное улучшение |
| Error Rate | <0.1% | 0.060% | В пределах нормы |

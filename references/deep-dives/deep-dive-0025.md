...
1. **Первый принцип**: Resilience-паттерны должны быть встроены в архитектуру изначально. Circuit break
2. **Второй принцип**: Архитектурная эволюция — естественный процесс. Не существует идеальной архитекту
3. **Третий принцип**: API design — это контракт на годы. Хороший API интуитивно понятен, консистентен 
4. **Четвёртый принцип**: Микросервисы — это не цель, а средство. Переход к микросервисам должен быть обос
...
|---------|----------|-----------|-------------|
| Throughput | 10k RPS | 17k RPS | В 3x лучше |
| Latency p50 | 29ms | 23ms | Улучшение |
| Latency p99 | 437ms | 58ms | Значительное улучшение |

...
2. **Второй принцип**: Микросервисы — это не цель, а средство. Переход к микросервисам должен быть обос
3. **Третий принцип**: Event-driven архитектура мощна, но сложна. Идемпотентность обработчиков, exactly
4. **Четвёртый принцип**: Caching — это всегда баланс между свежестью данных и производительностью. Инвали
...
|---------|----------|-----------|-------------|
| Throughput | 6k RPS | 97k RPS | В 7x лучше |
| Latency p50 | 96ms | 15ms | Улучшение |
| Latency p99 | 217ms | 30ms | Значительное улучшение |
| Error Rate | <0.1% | 0.095% | В пределах нормы |

...
2. **Второй принцип**: Data-intensive приложения требуют особого подхода к проектированию. Паттерны обр
3. **Третий принцип**: Database schema design — это искусство предсказывать будущие запросы. Нормализац
4. **Четвёртый принцип**: Event-driven архитектура мощна, но сложна. Идемпотентность обработчиков, exactly
...
|---------|----------|-----------|-------------|
| Throughput | 8k RPS | 11k RPS | В 16x лучше |
| Latency p50 | 94ms | 47ms | Улучшение |
| Latency p99 | 270ms | 84ms | Значительное улучшение |
| Error Rate | <0.1% | 0.042% | В пределах нормы |
